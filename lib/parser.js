...
let parseStruct = (typedef) => {
  ...
  for(let i=0;i<props.length;i++){
    ...
    if(parts.length === 2){
        props[i] = `&nbsp;&nbsp;&nbsp;&nbsp;${parts[0]} <span class="storage type numeric go">${parts[1]}</span>`
    }else if(parts.length >= 3){
      let tags = parts.slice(2).join(' ');

      tags = tags.replace(/\\"/g, "'");
      tags = tags.replace(/"/g, "`");
      tags = tags.replace(/'/g,'"');
      props[i] = `&nbsp;&nbsp;&nbsp;&nbsp;${parts[0]} <span class="storage type numeric go">${parts[1]}</span><span class="string quoted raw go"> ${tags}</span>`
    }
  ...
  console.log('new typedef', typedef);
